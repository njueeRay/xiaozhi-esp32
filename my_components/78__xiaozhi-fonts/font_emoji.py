...
def get_emoji_png(emoji_utf8, size):
    ...
    # 添加绝对路径
    image_converter_png = f"F:\\Project\\GitHub\\esp32-project\\xiaozhi-esp32\\scripts\\Image_Converter\\output\\emoji_{emoji_utf8}_{size}.png"
    
    if os.path.exists(image_converter_png):
        # 如果存在，直接复制到build目录
        import shutil
        shutil.copy(image_converter_png, png_path)
        print(f"已从 {image_converter_png} 复制表情图片")
    elif not os.path.exists(png_path):
        # 如果目标文件不存在，且Image_Converter中也没有，则从SVG转换
        cairosvg.svg2png(
            url=svg_path,
            write_to=png_path,
            output_width=size,
        ...
        print(f"已从SVG转换生成表情图片 {png_path}")
